...
def test_max_multiplier_cap():
    """ה-config מתיר 10× — וזו התקרה בפועל (אין יותר תקרת-ברזל של 3). ההגנות מהתקרית שנשארות:
    SETTLE_UNKNOWN לא נספר כהפסד + בלם-יתרה 25%; ההכפלה עצמה עולה עד מה שהמשתמש הגדיר."""
    st = _state()
    ...
    # 9 × 1.5 = 13.5 → נחסם בתקרת ה-config (10), לא ב-3
