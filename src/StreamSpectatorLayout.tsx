...
/** קטעי קו: ירוק מעל אפס, אדום מתחת — עם נקודת מעבר על y=0. */
...
function ChartIdlePanel({ copy }: { copy: ChartIdleCopy }) {
  return (
    ...
    >
      ...
      <div>
        <div style={{ fontSize: 20, fontWeight: 750, lineHeight: 1.35, color: "var(--text)" }}>{copy.headline}</div>
        <div style={{ fontSize: 14, color: "var(--text-secondary)", marginTop: 10, lineHeight: 1.45 }}>{copy.sub}</div>
      </div>
    </div>
  );
}
...
function LiveCentsPill(props: { label: string; mid: number | null; accent: "up" | "down"; flatMarket?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 42, fontWeight: 800, fontVariantNumeric: "tabular-nums", color, textShadow: glow, lineHeight: 1.1 }}>
        {pxToCentsLabel(mid ?? undefined)}
      </div>
      {flatMarket ? (
        <div style={{ fontSize: 10, marginTop: 10, color: "var(--muted)", fontWeight: 600, letterSpacing: "0.04em" }}>Market only — no open position</div>
      ) : null}
    </div>
  );
}

export type StreamSpectatorLayoutProps = {
  /** "v2" applies improved pulse-orb / card spacing. Default: original layout. */
  variant?: "v2";
  ...
  /** Trimmed high/low for chart reference lines only (avoids stuck guides after one bad sample). */
...
export function StreamSpectatorLayout(props: StreamSpectatorLayoutProps) {
  const {
    variant,
    fitBroadcast,
    broadcastParentRef,
  ...
  return (
    <div
      className={`stream-trade-root stream-trade-root--spectator${fitBroadcast ? " stream-broadcast-fit" : ""}`}
      style={{
    ...
    >
      <style>
        {`
          .stream-window-bar-fill--elapsed {
          ...
          .stream-broadcast-fit footer { display: none; }
        `}
      </style>
      ...
      >
        <div>
          ...
          >
            ...
            >
              {isV2 ? "SPECTATOR V2" : "SPECTATOR"}
            </span>
          </div>
          <h1 style={{ margin: "8px 0 0", fontSize: 23, fontWeight: 600, lineHeight: 1.25 }}>{market?.title ?? "Loading…"}</h1>
          <p
            style={{
              margin: "8px 0 0",
              fontSize: 13,
          ...
          >
            {isV2
              ? <>Improved layout — run P&amp;L, session clock, round history. Use <code style={{ color: "var(--accent-bright)" }}>?stream=5</code> or <code style={{ color: "var(--accent-bright)" }}>/stream/spectator-v2</code>.</>
              : <>English overlay — run P&amp;L, one session clock, round history. Use <code style={{ color: "var(--accent-bright)" }}>?stream=3</code> or{" "}<code style={{ color: "var(--accent-bright)" }}>/stream/spectator</code>.</>
            }
          </p>
        </div>
      ...
      <BroadcastFit enabled={fitBroadcast} parentRef={broadcastParentRef} contentRef={broadcastContentRef}>
        {err ? (
          <div style={{ padding: 12, borderRadius: 8, background: "var(--down-muted)", color: "var(--text)", marginBottom: 20 }}>{err}</div>
        ) : null}
        ...
        <section
          ...
          style={{
            ...
            display: "flex",
            flexWrap: "wrap",
            gap: fitBroadcast ? 12 : (isV2 ? 20 : 18),
            alignItems: "stretch",
            justifyContent: "space-between",
          }}
        >
          <div style={{ flex: "2 1 280px", minWidth: 0 }}>
            ...
            >
              <div style={{ fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--muted)", marginBottom: 8 }}>Bot mode</div>
              <div
                style={{
              ...
              {showHotStreak ? (
                <div style={{ marginTop: 12, fontSize: 13, fontWeight: 700, color: "#fbbf24", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ flex: isV2 ? "1.4 1 240px" : "1.4 1 260px", minWidth: 0 }}>
            <div
              style={{
                height: "100%",
                minHeight: 140,
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12, flexWrap: "wrap", marginBottom: 8 }}>
                <span style={{ fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "#fde68a", fontWeight: 800 }}>Run time</span>
                <span
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12, flexWrap: "wrap" }}>
                <span style={{ fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--muted)", fontWeight: 700 }}>Market window</span>
              ...
              <div style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 6 }}>Time left · bar shows elapsed % of this window</div>
              <div className="stream-window-bar" aria-hidden>
                <div className="stream-window-bar-fill stream-window-bar-fill--elapsed" style={{ width: `${windowElapsedPct}%` }} />
              </div>
          ...
          {/* v2: pulse orb on its own centered row to prevent collision with cards */}
          <div
            style={isV2
              ? { flex: "1 0 100%", display: "flex", justifyContent: "center", padding: "8px 0 4px" }
          ...
          >
            ...
            >
              <div style={{ position: "relative", zIndex: 1, textAlign: "center", padding: "10px 10px 8px" }}>
                <div style={{ fontSize: 11, letterSpacing: "0.05em", textTransform: "uppercase", color: "var(--muted)", fontWeight: 600, marginBottom: 5, lineHeight: 1.25, maxWidth: 148 }}>
        ...
        >
          ...
          <StreamBlock
            label="Unrealized %"
            sub="This market window · open trade"
          ...
          <StreamBlock
            label="Run P&amp;L"
            sub="Net since this bot run started"
          ...
          >
            <div style={{ fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--muted)", fontWeight: 700, lineHeight: 1.2, marginBottom: 6 }}>
              Win rate
              <br />
        ...
        <section id="stream-stats" aria-label="Run P&amp;L and round history">
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "flex-end", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
            <div>
              <div style={{ fontSize: 13, letterSpacing: "0.06em", textTransform: "uppercase", color: "#fde68a", fontWeight: 800 }}>Cumulative run P&amp;L (USD)</div>
              <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 4, maxWidth: 520, lineHeight: 1.35 }}>
                Net equity vs. when semi/auto was enabled — history from the engine after refresh; live points still append in this tab.
              </div>
            </div>
            {runUsdSessionStats && runPnlSeries.length > 0 ? (
              ...
              >
                <span>
                  <span style={{ color: "#fbbf24", fontWeight: 800, marginRight: 6 }}>▲ High</span>
                ...
                <span>
                  <span style={{ color: "#f87171", fontWeight: 800, marginRight: 6 }}>▼ Low</span>
                ...
                <span>
                  <span
                    style={{
                      color: runUsdSessionStats.last >= 0 ? RUN_PNL_GREEN : RUN_PNL_RED,
                      fontWeight: 800,
                      marginRight: 6,
                    }}
                  >
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: fitBroadcast ? 10 : 14, alignItems: "stretch" }}>
            <div
              className="stream-run-chart-shell"
              style={{
                flex: "3 1 320px",
                minWidth: 0,
                width: "100%",
                height: fitBroadcast ? 240 : 320,
                overflow: "hidden",
              }}
            >
              {stratCfg?.mode === "off" && chartIdleCopy ? (
                ...
                >
                  ...
                  <div style={{ fontSize: 13, color: "var(--muted)" }}>Sampling starts as soon as data flows.</div>
                </div>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={runPnlSeries} margin={{ top: 10, right: 8, bottom: 6, left: 4 }}>
            ...
            <aside
              ...
              style={{
                ...
                minWidth: 180,
                maxWidth: 320,
                maxHeight: fitBroadcast ? 240 : 320,
                overflow: "auto",
                padding: "12px 14px",
                borderRadius: 12,
            ...
            >
              ...
              {roundOutcomes.length === 0 ? (
                ...
                roundOutcomes.map((r) => (
                  <div key={r.id} className="stream-history-round">
                    <span
                      className="stream-history-dot"
        ...
        <footer style={{ marginTop: 24, fontSize: 12, color: "var(--muted)" }}>
          ...
          <div>
            <strong style={{ color: "var(--text-secondary)" }}>Classic</strong> (<code style={{ color: "var(--accent-bright)" }}>?stream=1</code>) ·{" "}
            <strong style={{ color: "var(--text-secondary)" }}>Showcase</strong> (<code style={{ color: "var(--accent-bright)" }}>?stream=2</code>) ·{" "}
            {isV2 ? (
              <><strong style={{ color: "var(--text-secondary)" }}>Spectator</strong> (<code style={{ color: "var(--accent-bright)" }}>?stream=3</code>). This page: <code style={{ color: "var(--accent-bright)" }}>?stream=5</code></>
            ) : (
              <><strong style={{ color: "var(--text-secondary)" }}>Spectator V2</strong> (<code style={{ color: "var(--accent-bright)" }}>?stream=5</code>). This page: <code style={{ color: "var(--accent-bright)" }}>?stream=3</code> · <code style={{ color: "var(--accent-bright)" }}>#stream-stats</code></>
            )}
            . Optional <code style={{ color: "var(--accent-bright)" }}>&amp;usd=0</code> hides $ readouts on Classic &amp; Showcase.
          </div>
        </footer>
      </BroadcastFit>
    </div>
